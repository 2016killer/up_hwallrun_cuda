...
local hwr_mustlookatwall = GetConVar('hwr_mustlookatwall')
local hwr_lifetime = GetConVar('hwr_lifetime')
...
function action:Check(ply)
    ...
    if hwr_mustlookatwall:GetBool() and ply:GetEyeTrace().HitNormal:Dot(wallForward) < 0.07 then
        return 
...
function action:Play(ply, mv, cmd, _, starttime)
    if CLIENT then return end
    if not ply.hwr_data then 
        return 
    end
    
    local movedata = ply.hwr_data
